...
const headers = { "Content-Type": "image/svg+xml; charset=utf-8", "Cache-Control": "public, max-age=300, s-maxage=3600, stale-while-revalidate=86400", "X-Content-Type-Options": "nosniff","Content-Security-Policy":"default-src 'none'; style-src 'unsafe-inline'; img-src https: data:" };

function svgResponse(svg: string, status: number, cache: "HIT" | "MISS" | "STALE", extra: Record<string, string> = {}) {
  return new NextResponse(svg, { status, headers: { ...headers, "X-RepoPulse-Cache": cache,"X-RepoPulse-Version":env.APP_VERSION, ...extra } });
}
...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    ...
    return svgResponse(renderErrorCard(message), 400, "MISS",{"X-RepoPulse-Request-Id":context.requestId});
  }
  ...
  if (cached?.fresh) {
    ...
    return svgResponse(cached.value, 200, "HIT",{"X-RepoPulse-Request-Id":context.requestId});
  }
  if(cached)return svgResponse(cached.value,200,"STALE",{"X-RepoPulse-Request-Id":context.requestId});
