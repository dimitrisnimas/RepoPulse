...
import { escapeXml, truncateText } from "@/server/cards/card-utils";
...
export function renderLanguagesCard(data: LanguagesCardData, options: LanguagesOptions): string {
  ...
  else if (options.layout === "compact") content += data.languages.map((language,index) => { const column = index % 2; const row = Math.floor(index / 2); const x = 24 + column * ((width - 40) / 2); const y = 83 + row * 42; return `<circle cx="${x+4}" cy="${y}" r="4" fill="${language.color}"/><text x="${x+14}" y="${y+4}" class="text" font-size="11">${escapeXml(truncateText(language.name,14))}</text><text x="${x+14}" y="${y+20}" class="muted" font-size="10">${language.percentage}%</text>`; }).join("");
  else content += data.languages.map((language,index) => { const y = 82 + index * (options.hideProgress ? 30 : 48); return `<circle cx="28" cy="${y}" r="4" fill="${language.color}"/><text x="40" y="${y+4}" class="text" font-size="11">${escapeXml(truncateText(language.name,22))}</text><text x="${width-24}" y="${y+4}" text-anchor="end" class="muted" font-size="11">${language.percentage}%</text>${options.hideProgress ? "" : `<rect x="24" y="${y+14}" width="${width-48}" height="6" rx="3" fill="${theme.progressBackground}"/><rect x="24" y="${y+14}" width="${Math.max(1,(width-48)*language.percentage/100)}" height="6" rx="3" fill="${language.color}"/>`}`; }).join("");
...
function renderDonut(data: LanguagesCardData, width: number, muted: string) {
  ...
  const arcs=donutSegments(data.languages).map((item)=>`<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${item.color}" stroke-width="16" stroke-dasharray="${circumference*item.percentage/100} ${circumference}" stroke-dashoffset="${-circumference*item.offset/100}" transform="rotate(-90 ${cx} ${cy})"/>`).join("");
  const legendX=Math.max(168,width*.48); const legend=data.languages.slice(0,6).map((item,index)=>`<circle cx="${legendX}" cy="${94+index*23}" r="4" fill="${item.color}"/><text x="${legendX+12}" y="${98+index*23}" class="text" font-size="10">${escapeXml(truncateText(item.name,15))}</text><text x="${width-24}" y="${98+index*23}" text-anchor="end" class="muted" font-size="10">${item.percentage}%</text>`).join("");
