...
async function repositoryActivity(name: string): Promise<PrivateRepositoryActivity> {
  ...
  return { name: data.repository.name, language: data.repository.primaryLanguage?.name ?? null, languageColor:data.repository.primaryLanguage?.color??null,totalCommits: history?.history?.totalCount ?? 0, lastCommitAt: history?.history?.nodes[0]?.committedDate ?? null };
}
export async function getPrivateActivity(now = new Date()): Promise<PrivateActivityData> {
  ...
  return { username: owner, about:privateConfig().about,repositories: available.sort((a,b)=>(b.lastCommitAt??"").localeCompare(a.lastCommitAt??"")), unavailable: settled.length-available.length, generatedAt: now.toISOString() };
}
