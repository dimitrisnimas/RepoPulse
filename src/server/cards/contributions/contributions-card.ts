...
export function renderContributionsCard(data:ContributionsCardData,options:ContributionsOptions):string{
 const{theme}=resolveTheme(options.theme);const width=options.width;const left=options.showWeekdays?48:24;const right=24;const gap=2;const cell=Math.max(5,Math.min(13,(width-left-right-gap*(data.weeks.length-1))/Math.max(1,data.weeks.length)));const gridWidth=data.weeks.length*(cell+gap)-gap;const top=91;const height=options.showLegend?210:184;const colors=[theme.contributions.empty,theme.contributions.level1,theme.contributions.level2,theme.contributions.level3,theme.contributions.level4];
 ...
 if(options.showLegend){const y=height-24;content+=`<text x="${left}" y="${y+4}" class="muted" font-size="9">Less</text>${colors.map((color,index)=>`<rect x="${left+29+index*15}" y="${y-6}" width="10" height="10" rx="2" fill="${color}"/>`).join("")}<text x="${left+108}" y="${y+4}" class="muted" font-size="9">More</text><text x="${left+gridWidth}" y="${y+4}" text-anchor="end" class="muted" font-size="9">${data.from} — ${data.to}</text>`;}
